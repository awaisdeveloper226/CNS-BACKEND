...
const uploadKeyOrProtect = asyncHandler(async (req, res, next) => {
  // Check for upload key first
  const uploadKey = req.headers['x-upload-key'];
  
  if (uploadKey && uploadKey === process.env.UPLOAD_SECRET_KEY) {
    const botUser = await User.findById(process.env.UPLOAD_BOT_USER_ID).select('-password');
    if (!botUser) {
      res.status(500);
      throw new Error('Upload bot user not configured. Contact admin.');
    }
    req.user = botUser;
    return next();
  }

  // No upload key — fall back to normal JWT protect
  let token;
